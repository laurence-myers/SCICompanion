...
* **Eliminated most `asm` fallbacks in the decompiler.** When the decompiler
  ...
  scripts now decompile with no `asm` fallbacks. A loop that a `break` leaves
  past its last jump back, with more loops in that part (King's Quest V,
  script 755), no longer stops the decompile of its script.
