...
GifFileType *DGifOpenFileName(const char *FileName, int *Error) {
	...
	// SCICompanion: use the Windows secure open with a deny-write share mode,
	// instead of POSIX open(), to match the rest of the app and avoid C4996.
	if (_sopen_s(&FileHandle, FileName, _O_RDONLY, _SH_DENYWR, 0) != 0) {
		if (Error != NULL) {
