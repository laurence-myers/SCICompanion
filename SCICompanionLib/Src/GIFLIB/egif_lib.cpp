...
/* SCICompanion: windows.h (via stdafx.h) already defines these; undo first to
...
GifFileType *EGifOpenFileName(const char *FileName, const bool TestExistence,
	...
	// SCICompanion: use the Windows secure open with a deny-read/write share
	// mode, instead of POSIX open(), to match the rest of the app and avoid C4996.
	errno_t openErr;
	if (TestExistence) {
...
int EGifPutScreenDesc(GifFileType *GifFile, const int Width, const int Height,
	...
	// SCICompanion (fork fix, orig commit 23aafb28): EGifSpew calls this with
	// GifFile->SColorMap itself as the ColorMap argument. Capture it here --
	// AFTER the early error returns above, so those paths leave the caller's map
	// in GifFile->SColorMap for EGifCloseFile to free (5.2.2 nulls it at the top
	// of the function, which would otherwise leak it). Then reuse it below
	// instead of building a new map object and leaking the caller's.
	ColorMapObject *incomingColorMap = GifFile->SColorMap;
	...
	if (ColorMap) {
		if (ColorMap == incomingColorMap) {
			/* Our own map was passed back in (EGifSpew); reuse it, do not
			   rebuild -- rebuilding would leak the caller's map. */
			GifFile->SColorMap = incomingColorMap;
		} else {
			GifFile->SColorMap =
