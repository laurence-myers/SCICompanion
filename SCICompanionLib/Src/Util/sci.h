...
public:
  ...
  inline void Stop()
  {
	  ...
	  elapsed += lEnd.QuadPart - lStart.QuadPart;
	  active = false;
  }
  ...
  inline double GetElapsed()
  {
	  ...
	  return (static_cast<double>(elapsed) / static_cast<double>(lFreq.QuadPart));
  }
};
...
extern const TCHAR* g_szResourceSpecByType[(unsigned int)ResourceType::Max];
