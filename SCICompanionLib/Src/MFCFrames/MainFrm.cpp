...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWnd)
	...
	ON_UPDATE_COMMAND_UI(ID_SCRIPT_EXTRACTALLTEXT, OnUpdateShowIfGameLoaded)
	...
	ON_UPDATE_COMMAND_UI(ID_NEWROOM, OnUpdateShowIfGameLoaded)
	ON_UPDATE_COMMAND_UI(ID_NEWSCRIPT, OnUpdateShowIfGameLoaded)
	ON_UPDATE_COMMAND_UI(ID_EDIT_FINDINFILES, OnUpdateShowIfGameLoaded)
	...
	ON_UPDATE_COMMAND_UI(ID_COMPILEALL, OnUpdateShowIfGameLoaded)
	ON_UPDATE_COMMAND_UI(ID_SCRIPT_MANAGEDECOMPILATION, OnUpdateShowIfGameLoaded)
	ON_UPDATE_COMMAND_UI(ID_SCRIPT_VALIDATEALLSAIDS, OnUpdateShowIfGameLoaded)
	ON_COMMAND(ID_STOPDEBUG, OnStopDebugging)
	ON_UPDATE_COMMAND_UI(ID_RUNGAME, OnUpdateShowIfGameLoaded)
	ON_UPDATE_COMMAND_UI(ID_DEBUGGAME, OnUpdateShowIfGameLoaded)
...
void CMainFrame::_PrepareMainCommands()
{
	static const key_value_pair<UINT, int> c_mainIcons[] =
	{
		...
		{ ID_EDIT_FINDINFILES, IDI_DITHER },
		...
		{ ID_CONVERTSCRIPT, IDI_SCRIPT_CONVERT },
		{ ID_SCRIPT_MANAGEDECOMPILATION, IDI_FAKEEGO },
	};
...
bool CompileABunchOfScripts(AppState *appState, DependencyTracker *dependencyTracker)
{
	...
	strMessage << " Parse: " << fmt::format("{0:.2f}", (float)g_compileSyntaxParseTimer.GetElapsed()) << " seconds.";
	strMessage << " CodeGen: " << fmt::format("{0:.2f}", (float)g_compileCodeGenTimer.GetElapsed()) << " seconds.";
	strMessage << " Total I/O : " << fmt::format("{0:.2f}", (float)g_compileIOTimer.GetElapsed()) << " seconds.";
	strMessage << " Sym I/O: " << fmt::format("{0:.2f}", (float)g_compileDebugSymbolTimer.GetElapsed()) << " seconds.";
	strMessage << " .sco I/O: " << fmt::format("{0:.2f}", (float)g_compileObjFileTimer.GetElapsed()) << " seconds.";
	strMessage << " write I/O: " << fmt::format("{0:.2f}", (float)g_compileAppendTimer.GetElapsed()) << " seconds.";
