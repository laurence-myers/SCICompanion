...
(class Window of SysWindow
	...
	(method (setMapSet &tmp temp0)
		(= temp0 0)
		(if (!= -1 color) (|= temp0 $0001))
		(if (!= -1 priority) (|= temp0 $0002))
		(return temp0)
	)
	
	(method (move theLeft theRight)
		(+= left theLeft)
		(+= right theRight)
		(+= right theLeft)
		(+= bottom theRight)
	...
	(method (save)
		(= underBits
			(Graph 7 top left bottom right (self setMapSet:))
		)
	)
	
	(method (restore)
		(if underBits (Graph 8 underBits))
	)
	
	(method (inset theLeft theTop)
		(+= top theTop)
		(+= left theLeft)
		(-= bottom theTop)
		(-= right theLeft)
